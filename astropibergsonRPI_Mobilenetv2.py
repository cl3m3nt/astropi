...
def main():
    ...
    '''
    # Load AI Model
    logger.info("Loading AI Mobilenetv2 Model")
    mobilenetv2_model = load_model("Mobilenetv2_TF114")
    ...
    while (now_time < start_time + timedelta(seconds=120)):
        ...
        # RGB Prepprocessing for expected shape by Mobilenetv2 
        logger.info("RGB preprocessing")
        ndvi_rgb_image = get_ndvi_rgb(ndvi_image)

        '''
        # Do Inference with AI Model
        logger.info("Doing Inference with AI Model")
        prediction = make_inference(ndvi_rgb_image,mobilenetv2_model)
        '''

        # Do Inference with TFLite Model
        logger.info("Doing Inference with TFLite Model")
        ndvi_rgb_image = ndvi_rgb_image.astype('float32')
        prediction = make_tflite_inference(ndvi_rgb_image,mobilenetv2_interpreter)
